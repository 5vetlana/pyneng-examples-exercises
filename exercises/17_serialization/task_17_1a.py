...
def write_dhcp_snooping_to_csv(filenames, output):
    ...
    for file in filenames:
        with open(file, 'r') as src:
            ...
            for match in regex.finditer(src.read()):
                line['switch'] = file.split('_')[0]
                line['mac'] = match.group('mac')
                line['ip'] = match.group('ip')
                line['vlan'] = match.group('vlan')
                line['interface'] = match.group('intf')
                data.append(line)
                line = {}
    ...
    with open(output, 'w', newline = '') as dest:
        ...
        dest, fieldnames=list(data[0].keys()), quoting=csv.QUOTE_NONNUMERIC)
        writer.writeheader()
        for row in data:
            writer.writerow(row)
